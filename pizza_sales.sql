...
select round(avg(quantity),0) as avg_pizza_orders_perday from
...
(select sum(order_details.quantity * pizzas.price) as total_sales from order_details
...
join pizzas on pizzas.pizza_type_id = pizzas.pizza_type_id
join order_details on order_details.pizza_id = pizzas.pizza_id
